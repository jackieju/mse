...
static void Definition(void)
{
	if (Sym == identifierSym ||
		...
		if (Sym >= objectSym && Sym <= stringSym) {
			Type(type);
		}
		Expect(identifierSym);
	...
	while (Sym == identifierSym ||
		...
		switch (Sym) {
			...
			case PointSym:  
				...
				//pop
				if (!PopDigit(&op1, &type1, &dt1) || type1 != dtGeneral){
				         GenError(96);
				        return;
				}else
				            {
...
static void Creator(void)
{
	...
	while (Sym == LparenSym) {
		Get();
		while (Sym >= objectSym && Sym <= stringSym) {
			FormalParamList();
		}
		Expect(RparenSym);
	}
...
static void ClassFullName(char* szName)
{
	Expect(identifierSym);
...
bool cParser::doMove(long type1, long type2, long op1, long op2, TYPEDES& dt1, TYPEDES& dt2 ){
	...
	if (dt1.type != dtGeneral && dt2.type != dtGeneral){
		...
		PushDigit(op1, AMODE_OBJ|0x80, dt1);
	}
	return true;
}
